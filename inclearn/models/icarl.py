...
from inclearn.models.PKE_utils import compute_features_FeatCluster
from inclearn.models.PKE_utils import compute_features_FeatCluster_new
from inclearn.models.PKE_utils import pdist
...
class ICarl(IncrementalLearner):
    ...
    def _training_step_finetune(
        ...
        for epoch in range(initial_epoch, nb_epochs):
            ...
            for i, input_dict in enumerate(prog_bar, start=1):
                ...
                loss = self._forward_loss_baseTask(
                    training_network,
                    inputs,
                    targets,
                    memory_flags,
    ...
    def _training_step(
        self, task_id, train_loader, val_loader, initial_epoch, nb_epochs, record_bn=True, clipper=None
    ):
        best_epoch, best_acc = -1, -1.
        wait = 0
        ...
        loss_weight = math.sqrt((self._n_classes - self._task_size) / self._task_size) * 2.5
        ...
        for epoch in range(initial_epoch, nb_epochs):
            ...
            if task_id == 0:
                ...
                ref_model = copy.deepcopy(self._old_model)
                tg_model = copy.deepcopy(self._network)

                num_features = self._old_model.classifier.features_dim
                _n_train_data = self._n_train_data
                _device = self._device
                input_train, old_feat_train1, new_feat_train1, targets_train, memory_flags_all = compute_features_FeatCluster_new(
                    ref_model, tg_model, train_loader, _n_train_data, num_features, _device)

                targets_train = torch.tensor(targets_train).long()
                memory_flags_all = torch.tensor(memory_flags_all)
                old_feat_train = F.normalize(torch.tensor(old_feat_train1).float(), p=2, dim=1)
                new_feat_train = F.normalize(torch.tensor(new_feat_train1).float(), p=2, dim=1)


                mask_old_classes = targets_train.lt(self._n_classes - self._task_size)
                select_index = []
                for i in range(mask_old_classes.shape[0]):
                    if mask_old_classes[i]:
                        select_index.append(i)
                select_index = torch.tensor(select_index)
                input_train_oldclasses = torch.index_select(input_train, 0, select_index)
                old_feature_of_examplars = torch.tensor(torch.index_select(old_feat_train, 0, select_index)).float()
                new_feature_of_examplars = torch.tensor(torch.index_select(new_feat_train, 0, select_index)).float()
                dist_oldF = pdist(old_feature_of_examplars, squared=False)
                dist_newF = pdist(new_feature_of_examplars, squared=False)
                dist_oldF_flatten = dist_oldF.view(1, -1)
                dist_oldF_l2 = F.normalize(dist_oldF_flatten, p=2, dim=1)
                dist_newF_flatten = dist_newF.view(1, -1)
                dist_newF_l2 = F.normalize(dist_newF_flatten, p=2, dim=1)

                loss_PKE1_l2 = (dist_oldF_l2 - dist_newF_l2).pow(2).sum().sqrt()

                old_feat_train = torch.tensor(old_feat_train).float()
                dist_z = pdist(old_feat_train, squared=False)

                #  not using itself AND old classes when choosing nearest top_k sample
                mask_old = torch.tensor(targets_train) < (self._n_classes - self._task_size)
                mask_cls = []
                a = mask_old.int()
                for cls_i in range(self._n_classes):
                    mask_cls.append(1 - torch.clamp(mask_old.int(), max=1))
                mask_cls = torch.stack(mask_cls)
                mask_input = torch.clamp(mask_cls[targets_train].float() - torch.eye(self._n_train_data), min=0)

                # just not using itself when choosing the nearest top_k sample
                # mask_cls = []
                # for cls_i in range(self._n_classes):
                #     mask_cls.append(torch.ones((self._n_train_data,)))
                # mask_cls = torch.stack(mask_cls)
                # mask_input = torch.clamp(mask_cls[targets_train].float() - torch.eye(self._n_train_data), min=0)

                # find the image meets label requirements with nearest old feature
                ...
                start_idx = 0
                batch_size = self._batch_size  # 128
                for i, input_dict in enumerate(prog_bar, start=1):
                    # inputs = input_dict["inputs"]
                    # targets = input_dict["targets"]
                    # memory_flags1 = input_dict["memory_flags"]

                    if start_idx >= self._n_train_data:
                        print("Error!")
                        break

                    if start_idx + batch_size < self._n_train_data:
                        inputs = input_train[start_idx:start_idx + batch_size, :, :, :]
                        inputs_match = input_train[match_id[start_idx * top_k:(start_idx + batch_size) * top_k], :, :, :]
                        targets = targets_train[start_idx:start_idx + batch_size]
                        ref_features = torch.tensor(old_feat_train[start_idx:start_idx + batch_size]).float()
                        ref_match_features = old_feat_train[match_id[start_idx * top_k:(start_idx + batch_size) * top_k]]
                        mu_dim = batch_size
                        memory_flags = memory_flags_all[start_idx:start_idx + batch_size]
                    else:
                        inputs = input_train[start_idx:, :, :, :]
                        inputs_match = input_train[match_id[start_idx * top_k:], :, :, :]
                        targets = targets_train[start_idx:]
                        ref_features = old_feat_train[start_idx:]
                        ref_match_features =[match_id[start_idx * top_k:]]
                        mu_dim = self._n_train_data - start_idx
                        memory_flags = memory_flags_all[start_idx:]

                    start_idx += batch_size

                    outputs_temp = self._network(inputs.to(self._device))['raw_features']

                    outputs_soft_temp = F.softmax(outputs_temp, dim=1)
                    top2_prob, top2_index = torch.topk(outputs_soft_temp, 2)
                    top2_sub = abs(top2_prob[:, 0] - top2_prob[:, 1])

                    mu_1, mu_2 = 0.6, 0.4
                    mu_1_, mu_2_ = torch.tensor(mu_dim * [mu_1]), torch.Tensor(mu_dim * [mu_2])
                    mask = top2_index[:, 0] != targets.to(self._device)
                    top2_sub[mask] = 0
                    mu_1_ada = (mu_1_ - torch.clamp(top2_sub.cpu(), min=0, max=0.2)).unsqueeze(dim=1).to(self._device)
                    mu_2_ada = (mu_2_ + torch.clamp(top2_sub.cpu(), min=0, max=0.2)).unsqueeze(dim=1).to(self._device)
                    ...
                    loss = self._forward_loss_incrTask(
                        loss_weight,
                        loss_PKE1_l2,
                        training_network,
                        inputs,
    ...
    def _forward_loss_incrTask(
        ...
        **kwargs
        ...
        inputs_match = inputs_match.to(self._device)
        ref_features = ref_features.to(self._device)

        outputs_muldict = training_network(inputs)
        outputs_match_muldict = training_network(inputs_match)
        if gradcam_act is not None:
            outputs_muldict["gradcam_gradients"] = gradcam_grad
            outputs_muldict["gradcam_activations"] = gradcam_act

        loss = self._compute_loss(inputs, outputs_muldict, targets, onehot_targets, memory_flags)

        loss_PKE2 = self._compute_loss_forPKE(
            inputs, inputs_match, outputs_muldict, outputs_match_muldict, targets, onehot_targets, memory_flags, top_k, mu_1_ada, mu_2_ada)


        with torch.no_grad():
            outputs_match = outputs_match_muldict['logits']
            match_features = outputs_match_muldict['raw_features']
        outputs = outputs_muldict['logits']
        outputs_soft = F.softmax(outputs, dim=1)
        cur_features = outputs_muldict['raw_features']

        # loss_UCIR = nn.CosineEmbeddingLoss()(cur_features, ref_features.detach(), torch.ones(inputs.shape[0]).to(self._device))

        loss_sum = loss + loss_PKE2 + loss_PKE1_l2 * l_weight

        if not utils.check_loss(loss):
            raise ValueError("A loss is NaN: {}".format(self._metrics))

        # self._metrics["loss"] += loss.item()
        self._metrics["loss_PKE1_l2"] += loss_PKE1_l2.item()
        # self._metrics["loss_LUCIR"] += loss_LUCIR.item()

        return loss_sum

    def _forward_loss_baseTask(
        self,
        training_network,
        inputs,
        targets,
